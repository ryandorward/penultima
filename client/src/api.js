...
let requestTalk = ({dir, text}) => {    
  console.log('requestTalk',dir,text)
  socket.send(JSON.stringify({
    "talk": {...dir, 'message': text } 
  }));
};

let requestSimpleAction = (action) => {    
  console.log('requestSimpleAction',action)
  socket.send(JSON.stringify({
    "simpleAction": {
      action: action
    }
...
let requestDirectionalAction = ({action, dir}) => {
  console.log('requestDirectionalAction',action, dir)
  socket.send(JSON.stringify({
    "directionalAction": {
      action: action,
      ...dir
...
export { 
  connect, sendMsg, requestMove, requestUpdateAvatar, requestUpdateName, 
  requestPeerGem, requestMagicSpell, requestLook, requestTalk, 
  requestSimpleAction, requestDirectionalAction 
}